...
Sparse checkout limits the ops worktree to essential directories (`ops/` and `state/`). `sources/cache/` remains on the ops branch but is excluded from the default sparse checkout to minimize disk usage. Workers needing source content (e.g., `arm sources sync`, `arm dag context`) expand the sparse checkout on demand.
...
.armature/
  ...
  templates/
    ...
    story.json
    task.json
    decomposition-prompt.md    # prompt template for dag context
  config.json                  # repository-level configuration
  hooks/
    post-commit                # LOCAL-ONLY — hook templates installed to .git/hooks/; never committed
    post-merge
    pre-commit
  gates/                             # LOCAL-ONLY — arm gate run stdout/stderr; never committed
  review/                            # LOCAL-ONLY — reviewer assessment JSON input to arm review record; never committed
```

**Key invariant:** All mutations go through log ops. `state/` files are derived/materialized locally by each worker and never committed to the ops branch — this is essential to the merge-conflict-free guarantee. `sources/` files are derived from provider fetches and committed to the ops branch for shared caching, but are never directly mutated during normal operations. `gates/` and `review/` are worker-local sidecars (the durable records are `gate-evidence` and `assessment-attested` ops) and are listed in the ops `.gitignore`, as are `hooks/*.sh.template` — every bootstrap regenerates the templates from its own binary and is the only reader of them, so committing them would only let clones on different versions fight over the same paths. `arm bootstrap` commits just `ops/SCHEMA` and that gitignore on `_armature`, and only when they change, so scaffolding does not leave the ops worktree dirty. `ops/SCHEMA` is generated from the running binary, so its propagation is monotonic: it carries a scaffolding-version header and a clone rewrites it only when its own generator is at least that new, which keeps an older binary from committing a downgrade into a shared path.
...
| `cancelled` | Reverted or discarded | `transition` op (e.g., `dag revert`) |
...
```text
arm dag apply --plan <plan.json> [flags]
...
`dag apply` enforces these checks before writing any ops. All checks also run in `--dry-run` mode.
...
The plan file is the contract between the decomposing agent and `arm dag apply`:
...
3. arm sources sync
4. arm dag context > context.json
5. <AI agent reads context.json, produces plan.json>
6. arm dag apply --plan plan.json --dry-run
7. <Fix validation errors, re-run AI if needed>
8. arm dag apply --plan plan.json
...
**AI produces invalid plan:** `arm dag apply --dry-run` reports all errors. The operator fixes the plan (or re-prompts the AI with error messages) and retries. No state is written until validation passes.

**AI produces partial plan (subset of requirements):** Valid. `arm dag apply` creates whatever is in the plan. `arm dag summary` shows uncited source sections (requirements not covered by any node). The reviewer flags gaps. A second decomposition pass can add nodes to an existing DAG.
...
The alternative is `git reset --hard` on the ops branch, which is a standard git operation but destroys the audit trail. `arm dag revert` is preferred when traceability matters.

### Decomposition Prompt Template

Ships as `.armature/templates/decomposition-prompt.md`, overridable per-repo. The `arm dag context` command interpolates source content, constraints, and existing DAG into template markers (`{{SOURCES}}`, `{{EXISTING_DAG}}`, `{{PLAN_SCHEMA}}`, `{{CONSTRAINTS}}`).
...
Nodes created via `arm dag apply` have `provenance.confidence: "draft"`. Draft nodes are not claimable. The `dag summary` sign-off promotes `draft` to `verified` by recording a `dag-transition` op with the signing worker's ID.
...
| Human interactive | `dag summary`, `sources stale-review` | Interactive TUI | Always |
| Workflow | `dag context`, `dag apply`, `dag revert`, `import`, `confirm` | JSON | Never |
...
|---|---|---|
| `claim`, `transition`, `heartbeat`, `note`, `create`, `link`, `decision`, `reopen`, `dag apply`, `dag revert`, `merged`, `confirm` | ops worktree | ops worktree |
| `ready`, `render-context`, `validate`, `status`, `metrics`, `context-history`, `dag summary`, `sources stale-review` | ops worktree | (none) |
...
| `sources stale-review` | Interactive TUI | Per-node decision flow |
...
### `sources stale-review` Per-Node Flow
...
| `dag apply` partial push (network timeout) | Potential duplicate nodes on re-run | Batch ID (UUID) recorded in each op's payload; re-run detects existing batch by plan file hash; requires `--force` to proceed |
...
- `dag revert` via cancellation ops (double-entry), not history rewriting
...
| L4 | `dag context` prompt template quality is load-bearing | Low | Template ships with tested defaults and is overridable. `dag summary` gate catches structural issues. |
...
| L6 | `dag revert` emitting cancellation ops adds noise to the log | Low | Preferable to `git reset --hard` which loses audit trail. Noise is bounded (one op per reverted node). |
