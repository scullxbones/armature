...
| `context-report` | main.go, context_report.go | Price fixture-measured main-path CLI payloads by bytes and estimated tokens | **kept-evidence** | Diagnostic meter. json/agent stdout for list, ready, render-context, and review; agent-mode show (human RenderIssue); plus a fixture render-context bundle. Embedded testdata. bytes/4 matches token_budget. |
