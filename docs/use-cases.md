...
| `low_stakes_push_threshold` | After this many consecutive low-stakes ops (notes, heartbeats, decisions), Armature attempts a best-effort push of `_armature` (same Push / FetchAndRebase / Push sequence as high-stakes) and resets the pending-push counter. Below the threshold, those ops commit locally only. High-stakes ops still push immediately after commit: `claim`, `transition`, `assign`, `unassign`, `ready` when it claims, and `doctor --fix`. If the field is omitted, the builtin fallback is 5. A present `0` is out of range: `arm doctor` D10 fails. |
