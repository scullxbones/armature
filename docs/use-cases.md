...
When a worker transitions a task to `done`, Armature runs an automated **delivery gate** — a set of three checks that verify the work meets minimal quality and scope standards before marking the task complete. This prevents incomplete, out-of-scope, or untraceable changes from being marked done.
...
The `--skip-delivery-gate` flag bypasses all three checks:
...
When you skip the gate, the override is recorded in the transition op's `Payload.SkippedDeliveryGate` field as an audit trail for future review.
