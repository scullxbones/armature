...
- Add the narrow missing verbs: `arm sources repoint` (or accept a new URL on an existing source ID), `arm reopen --reason`, and a `--field` that errors on unknown names rather than printing nothing.
