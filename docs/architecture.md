...
| **Ops branch** | The `_trellis` orphan branch storing all coordination data (`.issues/` directory). Unprotected, directly pushable by all workers. |
...
| **Working memory** | The rendered context slice a worker receives when claiming a task — the output of `trls render-context`. Maps to CoALA's working memory. |
...
_trellis (unprotected)  — .issues/ directory, direct push by all workers
```

The `_trellis` branch is an orphan branch with no common ancestor with `main`. All `.issues/` content lives here. Workers push ops directly to this branch. Code lives on feature branches and merges to `main` via PR.
...
```
git worktree add --no-checkout .trellis/ origin/_trellis
cd .trellis/
git sparse-checkout set .issues/ops .issues/state
```

The worktree directory is `.trellis/` in the repository root. This is the default location; it can be overridden via git config (`trellis.ops-worktree-path`). The `.trellis/` directory is added to `.gitignore` on all branches.

Sparse checkout limits the ops worktree to essential directories (`ops/` and `state/`). `sources/cache/` remains on the ops branch but is excluded from the default sparse checkout to minimize disk usage. Workers needing source content (e.g., `trls sources sync`, `trls decompose-context`) expand the sparse checkout on demand.

### Single-Branch Fallback

If `_trellis` does not exist and `main` is directly pushable (no branch protection), the CLI uses single-branch mode. All `.issues/` content lives on `main` alongside code. Detection is automatic at `trls init` time. This preserves viability for solo developers and non-enterprise setups.

### Worktree Lifecycle

| Event | Action |
|---|---|
| `trls init` | Creates `_trellis` orphan branch if needed, sets up worktree, sparse checkout, `.gitignore` entry |
| `trls init --repair` | Re-creates worktree if stale or missing, re-installs hooks |
| Worker operation | CLI `cd`s to ops worktree internally, pulls, materializes, executes, commits, pushes |
| Worktree corruption | `trls init --repair` deletes and recreates the worktree from remote |

### Directory Structure (within `.trellis/` worktree)
...
When multiple `decision` ops target the same `topic`, **last-write-wins by timestamp**. During materialization, the decision with the latest timestamp takes precedence. Earlier decisions for the same topic are preserved in the log for auditability but do not appear in the active context (Layer 5). `trls validate` surfaces a warning (W8) when conflicting decisions exist for the same topic, and `trls dag-summary` displays all conflicting decisions for human resolution.
...
- `done` → `open`: PR rejected or work found to be insufficient. Emitted via `trls reopen <issue-id>`. The previous outcome is preserved in a `prior_outcomes` array on the materialized node, providing context for the next worker.
...
**Read-only commands** (`trls ready`, `trls render-context`, `trls validate`, `trls status`, `trls metrics`, `trls context-history`):
...
**Read-write commands** (`trls claim`, `trls transition`, `trls heartbeat`, `trls note`, `trls create`, `trls link`, `trls decompose-apply`):

```
1. cd ops-worktree && git pull          (sync ops)
2. Incremental materialize              (only new ops since checkpoint)
3. Execute command                      (append to own log in ops-worktree)
4. git add .issues/ops/<worker-id>.log  (in ops-worktree)
5. git commit                           (in ops-worktree)
6. git push (retry with pull --rebase)  (to _trellis)
```
...
2. git log <last_sha>..HEAD -- .issues/ops/    (ops branch HEAD, not code branch)
...
**Fallback 3 — explicit command:** `trls merged ISSUE_ID`. Manual, low friction.
...
Dependency cycles are detected via DFS during materialization. Cyclic links are flagged `"valid": false` in the materialized state. The original op is preserved in the log for auditability. `trls validate` also checks for cycles.

### Compaction (Deferred)

`trls compact` is designed but not implemented in v1. When implemented, it will rewrite all worker logs into `ops/compacted-<sha>.log` containing one synthetic op per issue (current state), then truncate individual logs of pre-compaction ops. History is preserved in git. Do not implement until materialization performance is measured to be a problem.
...
Nodes with `confidence: "inferred"` (brownfield imports) include `"requires_confirmation": true` and cannot be claimed until a human confirms via `trls confirm <node-id>`.
...
After claiming and pushing, the worker should pull again and re-materialize to confirm it won the race before investing work. One extra pull — cheap insurance. The `trls claim` command handles this internally: claim, push, pull, re-check.
...
`trls render-context` produces the exact context slice a worker receives when it claims a task. The algorithm is deterministic — given the same materialized state, it always produces the same output. This allows human reviewers to spot-check any node and see exactly what an agent will see.
...
Advisory, not enforced. Proxy metric: `chars / 4`. Target range: 650–1,600 tokens per task context. Default budget from `.issues/config.json`, overridable per-invocation with `--budget <n>`.
...
Flags:
  ...
  --template <path>      Override decomposition prompt template
                         (default: .issues/templates/decomposition-prompt.md)
...
Exit codes: 0 success, 1 no sources registered, 2 source cache missing (run trls sources sync).
```
...
```
trls decompose-apply <plan.json> [flags]
...
Imported nodes with `inferred` confidence appear in `ready.json` with `requires_confirmation: true`. Workers cannot claim them until a human confirms via `trls confirm <node-id>`.
...
The plan file is the contract between the decomposing agent and `trls decompose-apply`:
...
```
1. trls sources add <prd-url>
2. trls sources add <arch-doc-url>
3. trls sources sync
4. trls decompose-context > context.json
5. <AI agent reads context.json, produces plan.json>
6. trls decompose-apply plan.json --dry-run
7. <Fix validation errors, re-run AI if needed>
8. trls decompose-apply plan.json
9. trls dag-summary          # human reviews, interactive sign-off
```

Sign-off via `trls dag-summary` is the governance gate for DAG structure. There is no PR on the ops branch — the ops branch must remain unprotected for low-latency coordination.

### Brownfield Flow

```
1. trls import existing-tasks.csv --format csv --source src-001
   # Creates nodes with confidence=inferred
2. trls sources add <prd-url>
3. trls sources sync
4. trls decompose-context --existing-dag > context.json
   # Context includes imported nodes for AI to reconcile
5. <AI produces plan.json, referencing or extending imported nodes>
6. trls decompose-apply plan.json --dry-run
7. trls decompose-apply plan.json
...
10. trls confirm <node-id>    # promotes inferred → verified
```

### Partial and Failed Decomposition

**AI produces invalid plan:** `trls decompose-apply --dry-run` reports all errors. The operator fixes the plan (or re-prompts the AI with error messages) and retries. No state is written until validation passes.

**AI produces partial plan (subset of requirements):** Valid. `trls decompose-apply` creates whatever is in the plan. `trls dag-summary` shows uncited source sections (requirements not covered by any node). The reviewer flags gaps. A second decomposition pass can add nodes to an existing DAG.
...
The alternative is `git reset --hard` on the ops branch, which is a standard git operation but destroys the audit trail. `trls decompose-revert` is preferred when traceability matters.

### Decomposition Prompt Template

Ships as `.issues/templates/decomposition-prompt.md`, overridable per-repo. The `trls decompose-context` command interpolates source content, constraints, and existing DAG into template markers (`{{SOURCES}}`, `{{EXISTING_DAG}}`, `{{PLAN_SCHEMA}}`, `{{CONSTRAINTS}}`).
...
```json
{
  ...
  "cached_sha": "abc123",
  "cache_path": ".issues/sources/cache/src-001.md"
}
```
...
- `trls sources add <url>` — registers and fetches initial content
- `trls sources sync` — checks provider version metadata, fetches if changed
- `trls sources verify` — validates fingerprints, flags stale nodes
...
All provider content is normalized to markdown on fetch and stored in `.issues/sources/cache/`. Mermaid diagrams embedded in Confluence pages are extracted as `<source-id>-<n>.mermaid` sidecar files. This is the abstraction boundary — everything downstream sees markdown regardless of provider.
...
During `trls sources sync`, the CLI compares the provider's current version metadata against the cached version. If changed, the CLI fetches the new content, recomputes the SHA, and emits a new `source-fingerprint` op.

**Cross-document consistency:** If a PRD version timestamp is newer than an architecture document version timestamp by more than a configurable threshold (default: 7 days), `trls dag-summary` surfaces a warning. Resolution is human responsibility.

### Credential Management

Stored in `~/.config/trls/providers.toml` — never in the repo:
...
Nodes created via `trls decompose-apply` have `provenance.confidence: "draft"`. Draft nodes are not claimable. The `dag-summary` sign-off promotes `draft` to `verified` by recording a `dag-transition` op with the signing worker's ID.

### Inferred Confidence

Nodes created via `trls import` have `provenance.confidence: "inferred"`. These require explicit per-node confirmation via `trls confirm <node-id>` before workers can claim them. Bulk promotion is not supported — this rate-limiting is intentional to force human review of imported work items.
...
`trls dag-summary` surfaces coverage metrics: cited nodes / total nodes. Uncited nodes are named individually (not just counted). The reviewer must acknowledge each uncited node by name in the sign-off, creating attribution in the audit log.

`dag-summary.md` is committed to the ops branch as an audit artifact. It is generated by `trls dag-summary` and committed alongside the sign-off ops.
...
| Status | Meaning | Set By |
|---|---|---|
| `done` | Worker believes work is complete, PR submitted | Worker via `trls transition` |
| `merged` | Code confirmed on main via PR merge | CLI auto-detection during materialization |
...
4. **Explicit command** — `trls merged ISSUE_ID` (manual fallback)
...
**Abandoned PRs:** A task stuck at `done` with no PR merge triggers a staleness check. Configurable threshold (default: N days after `done` with no merge detected). Same pattern as expired claims — surfaced via `trls status`.
...
`trls init` installs hooks from `.issues/hooks/` into `.git/hooks/`. **Git hooks are convenience, never enforcement.** Every action a git hook performs is also available as an explicit CLI command. The system is correct without git hooks installed.

**Important distinction:** Git hooks (this section) are convenience automation for heartbeats, commit-message stamping, and merge promotion. Pre-transition verification hooks (section 12) configured as `required: true` in `.issues/config.json` are **enforcement** — the CLI refuses to emit a `done` transition if a required verification hook fails, regardless of whether git hooks are installed. These are separate mechanisms with different trust models.

| Hook | Trigger | Action |
|---|---|---|
| `post-commit` | After any commit on a feature branch | Auto-emits heartbeat if worker has active claim; pushes to ops branch |
| `post-merge` | After `git pull` brings new commits to main | Runs `trls materialize` to pick up newly merged tasks and auto-promote `done` → `merged` |
...
- `trls init` installs hooks from version-controlled templates in `.issues/hooks/`
- `trls init --no-hooks` skips hook installation
- `trls init --repair` re-installs if hooks were removed or corrupted
...
Verification hooks are configured in `.issues/config.json` on the ops branch, shared by all workers:
...
`trls init` checks for well-known files and proposes defaults:
...
Verification is strictly two-phase: verify in code worktree, then record in ops worktree. The CLI discovers the code worktree by walking up from `cwd` to find the `.git` directory (standard git behavior). The developer runs `trls transition` from within their code worktree. The CLI runs hooks in `cwd`, then switches to the ops worktree internally to record the transition.
...
Implicit in all commands. Explicit `trls sync` exists for diagnostics, scripting, and batch operations.
...
Behavior:
  ...
  2. If protected: create _trellis orphan branch (if needed), set up ops worktree
  3. If not protected: use single-branch mode
  4. Auto-detect project type, propose verification hooks
  5. Write .issues/config.json
...
`trls worker-init` (also run as part of `trls init`) generates a UUID and writes it to repo-local git config:
...
```
git config --local trellis.ops-worktree-path .trellis/
```
...
Displays per affected node: the source change summary (section-level diff from provider version history where available), the node's citation, and the full `trls render-context` output. Options per node: confirm citation valid, flag for re-decomposition, skip. Each response is a log op.
...
```
$ trls version                    # verify trls is available
$ trls worker-init --check        # verify worker identity
# If no worker ID: $ trls worker-init
```
...
Step 2: Get Context
  $ trls render-context <issue-id> --format=agent
...
Step 4: Execute
  ...
  $ trls note <issue-id> "Discovered X, handled by Y"

  If you encounter an undocumented decision point:
  $ trls decision <issue-id> --topic="<topic>" --choice="<choice>" --rationale="<why>" --affects="<scope-glob>"
...
Step 6: Complete (ops)
  $ trls transition <issue-id> done --outcome="<one-line summary>"
...
Cannot complete a task:
  $ trls transition <issue-id> blocked --outcome="<why blocked>"
  Return to Step 1.

Wrong task claimed:
  $ trls transition <issue-id> open --outcome="Unclaimed: <reason>"
  Return to Step 1.

PR rejected / work insufficient:
  $ trls reopen <issue-id> --outcome="PR rejected: <reason>"
...
| Error | Cause | Hint |
|---|---|---|
| `ops branch not found` | `_trellis` branch missing | `run trls init` |
| `ops worktree desync` | Local ops worktree is behind or corrupted | `run trls sync` or `trls init --repair` |
| `stale worktree` | Worktree path exists but points to wrong branch | `run trls init --repair` |
...
| Task stuck at done if PR abandoned | Downstream permanently blocked | Staleness check (no merge within N days of done); surfaced via `trls status` |
| Manual commits to ops branch | Unexpected state | CLI ignores non-.issues/ files; contributing guide documents convention |
...
```
trls render-context <issue-id> --at <sha>
```
...
```
trls materialize --exclude-worker <worker-id>
```
...
Run `trls validate` at different ops branch commits using `git bisect` to find when a structural error (cycle, orphan link, missing field) was introduced.

### Sprint Bookmarks

Git tags on the ops branch mark sprint boundaries. Example: `git tag sprint-12 _trellis` at sprint close. Enables reconstructing state at any sprint boundary.

### Code-Ops Correlation

The transition op records `branch` and `pr` fields, bridging the ops history (on `_trellis`) with the code history (on `main` and feature branches). Given an ops commit, you can find the corresponding code PR. Given a code commit, the issue ID in the commit message (from the `prepare-commit-msg` hook) links back to the ops.
...
|---|---|---|---|
| Repository | `.issues/config.json` (on ops branch) | Shared by all workers | Templates, pre-transition hooks, token budget, TTL defaults, staleness thresholds |
| Worker | Repo-local git config | Per-worktree | `trellis.worker-id`, `trellis.ops-worktree-path` |
| User | `~/.config/trls/providers.toml` | Per-machine | Provider credentials (Confluence, SharePoint auth) |
...
```json
{
  ...
  "templates": {
    "task": ".issues/templates/task.json",
    "story": ".issues/templates/story.json",
    "epic": ".issues/templates/epic.json"
  }
}
```
...
**v1: Separate instances per repo (Option A).** Each repo has its own `trls init`, its own ops branch, its own DAG. Cross-repo dependencies are tracked manually (notes on tasks). The CLI has no awareness of other repos.
...
Implementation path: `trls init --ops-repo=acme/trellis-ops` configures the ops worktree to point at the external hub repo. All CLI commands operate against the hub. The code worktree is the current repo.
...
- Dual-branch architecture: `_trellis` (unprotected, orphan) + `main` (protected, code)
...
| L12 | Merge detection coverage degrades silently without `prepare-commit-msg` hook | Low | W9 validation warning monitors coverage. `trls validate` surfaces low coverage rate. Fallback detection methods remain available. |