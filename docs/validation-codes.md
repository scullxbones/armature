...
**Trigger:** Two non-terminal ready-eligible issues (`task`, `bug`, `feature`, or `story`) have overlapping scope globs, neither is an ancestor/descendant of the other, and neither blocks the other (directly or transitively). Comparisons are graph-wide, including cross-parent pairs. Epics and terminal issues are ignored. Story/feature parents that have descendants are also ignored (their stored scope is a rollup; live children compete instead) — **unless the parent is itself explicitly claimed** with a claim still inside its TTL, in which case a worker is editing its declared files right now and it competes on that scope like any other live issue. (A parent merely promoted to `in-progress` by a child's claim has no claimant of its own and stays passive, as does one whose claim has expired.) Leaf stories/features with no descendants still compete on declared scope. Parent/child pairs are suppressed because a parent's scope is the union of its descendants.
