...
func TestRunRepoSetupIdempotentDualBranchMode_REQ_SB_T9(t *testing.T) {
	...
	// Scoped to the migrated data directories/files: .gitignore, hooks/*.template, and
	...
	for _, line := range strings.Split(strings.TrimSpace(statusOut), "\n") {
		...
		if strings.Contains(line, ".gitignore") || strings.Contains(line, "hooks/") || strings.Contains(line, "ops/SCHEMA") {
			continue // scaffolding regenerated by every run, never committed even pre-fix
		}
