...
func newCmdInDir(dir string, args ...string) *exec.Cmd {
...
func TestReviewPrepare_CoordinatorWaveScope(t *testing.T) {
	...
	waveBaseSHACmd := newCmdInDir(repo, "rev-parse", "HEAD")
	...
	taskASHACmd := newCmdInDir(repo, "rev-parse", "HEAD")
	...
	taskBSHACmd := newCmdInDir(repo, "rev-parse", "HEAD")
...
func TestReviewPrepareCommand_FindsActivityLogInDeliveryWorktree_REQ_EXECEV_C1(t *testing.T) {
	...
	baseCmd := newCmdInDir(worktreeDir, "rev-parse", "HEAD~1")
	baseOut, err := baseCmd.Output()
	require.NoError(t, err)
	base := strings.TrimSpace(string(baseOut))
	headCmd := newCmdInDir(worktreeDir, "rev-parse", "HEAD")
	headOut, err := headCmd.Output()
	require.NoError(t, err)
	head := strings.TrimSpace(string(headOut))
