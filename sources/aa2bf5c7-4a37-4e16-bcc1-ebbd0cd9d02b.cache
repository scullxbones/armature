...
`main.go` already auto-sets `--non-interactive` when `--format=agent`, non-TTY, or `$GEMINI_CLI`/`$TERM=dumb`. Rule: this is the **one** TTY-detection mechanism. Commands must read `--non-interactive`, not hand-roll their own `tui.IsTerminal()` check. `dagsum.go` and `stalereview.go` were fixed to read the flag as part of this item's audit; `bootstrap.go` was the last holdout (it bypasses the root `PersistentPreRunE` — `config.ResolveContext` would fail on an unbootstrapped repo — so it never picked up the flag that `PersistentPreRunE` sets). Rather than carve out a permanent exception, the detection logic was factored out of `main.go`'s `PersistentPreRunE` into a shared `applyTTYDetectionPolicy(cmd *cobra.Command)` helper (still defined in `main.go`, still the only place that calls the terminal-detection primitive), and `bootstrap.go`'s own `PersistentPreRunE` override now calls that helper directly instead of checking the terminal itself. `grammar_test.go`'s file-scan allowlist for the raw terminal-detection call is `main.go` only — `bootstrap.go` was removed from it once the consolidation landed. `accept_citation.go`'s `--ci` flag is a duplicate of `--non-interactive`; its removal is `NXTTN-S2`'s job (census cut), not this item's.
