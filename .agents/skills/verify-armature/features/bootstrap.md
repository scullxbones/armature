...
- Git hooks installed into `$TARGET/.git/hooks/` independently of `--with-hooks`: `pre-commit`, `post-commit`, `post-merge`, `prepare-commit-msg`. Occupied unmanaged hook paths are skipped (`skipped_hooks`; `cmd/armature/bootstrap.go` `installHooks`). `--with-hooks` governs *harness* hook config only, not these.
...
ls "$TARGET/.git/hooks/" | rg -v '\.sample$'   # pre-commit post-commit post-merge prepare-commit-msg
```
