...
func TestBlobOIDAtRev(t *testing.T) {
	t.Parallel()
	repo := initTestRepo(t)
	c := adapters.New(repo)
	...
	require.NoError(t, os.WriteFile(filepath.Join(repo, "alpha.txt"), []byte("hello"), 0o600))
	gitRun("add", "alpha.txt")
	gitRun("commit", "-m", "add alpha")
	...
	blobCmd := exec.CommandContext(context.Background(), "git", "-C", repo, "rev-parse", "HEAD:alpha.txt")
	blobOut, err := blobCmd.Output()
	require.NoError(t, err)
	wantOID := strings.TrimSpace(string(blobOut))

	oid, ok, err := c.BlobOIDAtRev(sha, "alpha.txt")
	require.NoError(t, err)
	require.True(t, ok)
	assert.Equal(t, wantOID, oid)
}

func TestBlobOIDAtRev_MissingPath(t *testing.T) {
	t.Parallel()
	repo := initTestRepo(t)
	c := adapters.New(repo)

	oid, ok, err := c.BlobOIDAtRev("HEAD", "does-not-exist.txt")
	require.NoError(t, err)
	assert.False(t, ok)
	assert.Empty(t, oid)
}

func TestBlobOIDAtRev_InvalidRev(t *testing.T) {
	t.Parallel()
	repo := initTestRepo(t)
	c := adapters.New(repo)

	_, _, err := c.BlobOIDAtRev("deadbeefdeadbeefdeadbeefdeadbeefdeadbeef", "alpha.txt")
	assert.Error(t, err)
}

func TestCommitDiffTreeStatus(t *testing.T) {
	t.Parallel()
	repo := initTestRepo(t)
	c := adapters.New(repo)
	...
	entries, err := c.CommitDiffTreeStatus(sha)
	require.NoError(t, err)
	require.Len(t, entries, 1)
...
func TestCommitDiffTreeStatus_InvalidSHA(t *testing.T) {
	t.Parallel()
	repo := initTestRepo(t)
	c := adapters.New(repo)

	_, err := c.CommitDiffTreeStatus("deadbeefdeadbeefdeadbeefdeadbeefdeadbeef")
	assert.Error(t, err)
}
